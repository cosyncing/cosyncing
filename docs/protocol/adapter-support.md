...
| F13 agent-to-user artifacts | partial: maps native SendUserFile transcript records, but local CLI/Drive exposes no callable delivery tool | n/a: no exact session-qualified delivery route; shared cwd outbox fails closed | full: session-qualified send_file plus exact native write events | full: session-qualified bridge send-file route |
