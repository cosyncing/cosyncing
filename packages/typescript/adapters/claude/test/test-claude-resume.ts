 ...
 *
 *   bun run packages/typescript/adapters/claude/test/test-claude-resume.ts      (exit 0 = all pass; live half may SKIP)
 */
export {};
...
async function livePermission(): Promise<void> {
  ...
  const { ClaudeResumeConnection, claudeStores: storesFn, claudeModelOptions: modelsFn } = await import('../src/index.ts');
