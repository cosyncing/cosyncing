...
/** Broker-owned persistence effects used by the adapter-owned managed-server lifecycle. */
export interface ManagedOpencodeServeState {
  readOwnership(): OpencodeServeOwnership | null;
  writeOwnership(identity: ProcessIdentity, baseUrl: string): void;
  clearOwnership(): void;
  recordStartFailure(detailCode: string, capturedOutput: string): void;
  clearStartFailure(): void;
}

const NO_MANAGED_STATE: ManagedOpencodeServeState = {
  readOwnership: () => null,
  writeOwnership: () => undefined,
  clearOwnership: () => undefined,
  recordStartFailure: () => undefined,
  clearStartFailure: () => undefined,
};
let managedState: ManagedOpencodeServeState = NO_MANAGED_STATE;

/** Install broker persistence without making this provider package depend on broker internals. */
export function configureManagedOpencodeServeState(state: ManagedOpencodeServeState): void {
  managedState = state;
}
...
function recordStartFailure(detailCode: string, output = ''): void {
  try { managedState.recordStartFailure(detailCode, output); } catch {
    /* diagnostics are best-effort and must never break the managed runtime */
  }
}

function clearStartFailure(): void {
  try { managedState.clearStartFailure(); } catch {
    /* diagnostics are best-effort and must never break the managed runtime */
  }
}
...
/** Broker persistence is fail-closed: unavailable or malformed state never proves ownership. */
function readOwnershipRecord(): OpencodeServeOwnership | null {
  try {
    return managedState.readOwnership();
  } catch {
    return null;
  }
}

/** Persist ownership best-effort; a failure means a later broker preserves the process as unproven. */
function writeOwnershipRecord(identity: ProcessIdentity, baseUrl: string): void {
  try {
    managedState.writeOwnership(identity, baseUrl);
  } catch {
    /* ownership proof degrades safely to preserve */
  }
}

function clearOwnershipRecord(): void {
  try {
    managedState.clearOwnership();
  } catch {
    /* already absent or broker state unavailable */
  }
}
