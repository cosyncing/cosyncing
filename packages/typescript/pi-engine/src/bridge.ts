...
function bridgeRunSummary(ev: any, dialect: PiDialect = PI_DIALECT): AgentMessage {
  ...
  return {
    type: 'run-summary',
    key: String(ev.key ?? `${dialect.eventKeyNamespace}${ev.turnId ?? ''}`),
    ...
    totalRuntimeMs,
    tokens: ev.tokens,
    source: ev.source ?? dialect.eventSources.bridge,
  };
}
...
export class PiBridgeConnection implements SessionConnection {
  ...
  constructor(info: SessionInfo, private readonly dialect: PiDialect = PI_DIALECT) {
    this.info = info;
    this.lastSeen = 0;
  }
  ...
  ingest(ev: any): void {
    ...
    switch (ev?.t) {
      ...
      case 'run-summary':
      case 'run': {
        const m = bridgeRunSummary(ev, this.dialect);
        this.appendHistory(m);
        this.emit(m);
        return;
      }
  ...
  getHistorySourceIdentity(): HistorySourceIdentity {
    return {
      sourceId: `${this.dialect.eventSources.bridge}:${this.info.id}`,
  ...
  private writeInboxFile(file: FileInput): string {
    ...
    if (!cwd) throw new Error(`${this.dialect.displayName} attachment delivery requires a workspace.`);
    ...
    if (file.brokerPath) {
      ...
      if (dirname(brokerPath) !== inbox || !existsSync(brokerPath)) {
        throw new Error(`${this.dialect.displayName} rejected an untrusted broker attachment path.`);
      }
      return brokerPath;
    }
    if (typeof file.data !== 'string') throw new Error(`${this.dialect.displayName} attachment bytes are missing.`);
    try {
      ...
      throw new Error(`${this.dialect.displayName} could not materialize attachment ${file.name}: ${String(error)}`);
    }
  }
  ...
  async respondPermission(requestId: string, decision: PermissionDecision): Promise<void> {
    if (decision === 'approve-rule') {
      throw new Error(`${this.dialect.displayName} does not support persistent approval rules through this connection`);
    }
...
export class PiBridgeRegistry {
  ...
  constructor(
    ...
    /** Dialect for connections this registry mints — omp bridges get omp labels/keys. */
    private readonly dialect: PiDialect = PI_DIALECT,
  ) {}
  ...
  hello(id: string, info: SessionInfo): PiBridgeConnection {
    ...
    const conn = new PiBridgeConnection(info, this.dialect);
