...
import type { DoctorSection } from '../installation/doctor.ts';
import type {
...
import {
  ...
  type SetupLanguage,
} from '../installation/setup-i18n.ts';
...
const ZH_HUMAN_TEXT: Readonly<Record<string, string>> = Object.freeze({
  ...
  'The installed Pi bridge is a receipt-proven older packaged version.': '已安装的 Pi bridge 是由收据证明归属的旧版发行包副本。',
  'Run setup or repair to refresh the package-owned Pi bridge.': '请运行 setup 或 repair，将归属发行包的 Pi bridge 更新到当前版本。',
  'The Pi bridge receipt does not prove ownership of the installed target.': 'Pi bridge 收据无法证明已安装目标的归属。',
  'Preserve the bridge and reconcile its install receipt before setup or repair.': '请保留该 bridge，并在运行 setup 或 repair 前修复其安装收据。',
  ...
  'The Pi bridge target has an unsafe filesystem type or symlinked path.': 'Pi bridge 目标的文件系统类型不安全，或路径中含有符号链接。',
  ...
  'Codex managed app-server socket is readable through the runtime control-socket alias.': 'Codex 托管 app-server 套接字可通过运行时控制套接字符号链接读取。',
  'Codex managed app-server socket points at a runtime socket that is no longer present.': 'Codex 托管 app-server 套接字指向的运行时套接字已不存在。',
  'Codex managed app-server socket is readable through an alias but cannot be read.': 'Codex 托管 app-server 套接字有符号链接，但无法读取该套接字。',
  'Codex managed app-server socket aliases something that is not a socket.': 'Codex 托管 app-server 套接字指向的不是套接字。',
