 ...
 *
 * Run: `bun run packages/typescript/broker/src/runtime.ts`  (or `bun run broker`)
...
import { Hub, type Client, type ManagedConn, type WireEvent } from './hub.ts';
import { authoritativeLiveOwners, overlayAuthoritativeOwner } from './roster-overlay.ts';
import {
  createRosterPublicationBoundary,
  NativeIncarnationPublicationAuthority,
} from './roster-publication.ts';
import { SharedDraftStore } from './draft-store.ts';
import {
  FsBrowseError,
...
import { DownloadRangeError, ifRangeMatches, parseDownloadRange } from './fs-download-range.ts';
import { PiBridgeRegistry } from './pi-bridge.ts';
import { ClaudeHooksRegistry } from './claude-hooks.ts';
import { HISTORY_WEBSOCKET_OPTIONS } from './ws-limits.ts';
import {
  acceptsGzip,
...
import { RosterRevisionStore } from './roster-revision.ts';
import {
  ArtifactStore,
  artifactCacheRoot,
  type AuthorizedArtifactInteraction,
} from './artifact-store.ts';
import { buildDiffRefMessage, INLINE_DIFF_CAP } from './diff-reference.ts';
import { assertR2ActionsSafe, consumeConfirmNonce, deriveSessionRevision, getR2Action, issueConfirmNonce, r2ActionAvailable, r2MaxBytes, reserveR2RateSlot, trustTierForAddress } from './r2-policy.ts';
import { runTranscriptExport } from './r2-export.ts';
import {
...
import { refreshSessionOptions } from './session-options.ts';
...
import { PairingHttpError, tokenHash, TransportPairingRegistry } from './transport-pairing.ts';
...
import {
  ...
  UploadError,
  UploadStaging,
} from './upload-staging.ts';
import { dispatchWakePush, WakePushError, WakePushRegistry } from './push-wake.ts';
import {
  aggregatedMachines,
...
} from './machine-aggregation.ts';
import { AttentionService } from './attention-service.ts';
import { AuthFailureAttentionTracker } from './security-attention.ts';
import { BrokerHealthMonitor, type BrokerHealthSnapshot } from './broker-health.ts';
import { BrokerHealthAttentionReconciler } from './broker-health-attention.ts';
import { DeviceWakeCoalescer } from './wake-coalescer.ts';
import {
  fetchTokdashQuota,
...
} from './tokdash-quota.ts';
import { AttentionReminderScheduler } from './attention-reminder-scheduler.ts';
import {
  isValidTimeZone,
  MAX_SCHEDULED,
...
import { resolveRuntimeCredentials, safeCredentialEqual } from './credentials.ts';
import { inspectDurableCorruptionEvidence, inspectDurableSchemas } from './durable-state.ts';
import { remoteFilesystemAllowed } from './client-control-boundary.ts';
import {
