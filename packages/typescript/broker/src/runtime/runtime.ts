...
import { TokdashReportService } from '../installation/tokdash-report-service.ts';
...
const tokdashReportService = new TokdashReportService({
  baseUrl: TOKDASH_URL,
  cache: tokdashReportCache,
  identityTtlMs: Math.max(0, envNumber('COSYNCING_TOKDASH_FINGERPRINT_TTL_MS', 5 * 60_000)),
});
...
server = Bun.serve<WsData>({
  ...
  async fetch(req, srv) {
    ...
    if (path === '/api/tokdash/report' && req.method === 'GET') {
      ...
      try {
        const { entry, servedFromCache } = await tokdashReportService.read(window);
