...
async function defaultRunUpgrade(options: {
  ...
  const [{ runUpgrade }, lifecycle] = await Promise.all([
    import('./release-upgrade.ts'),
    import('./broker-lifecycle.ts'),
  ]);
  const home = (await import('./setup-state.ts')).setupStateHome();
  const setupState = (await import('./setup-state.ts')).readSetupState(home);
  const service = (await import('./setup-state.ts')).isDurableServiceChoice(setupState.serviceChoice)
    ? (() => {
    ...
    try {
      ...
      if (args[0] === '--json') {
        ...
        if (dependencies.renderDoctorReport) {
          ...
          const { doctorColorEnabled, renderDoctorReport } = await import('./doctor.ts');
