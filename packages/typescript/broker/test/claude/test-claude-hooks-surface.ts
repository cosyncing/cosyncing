 ...
 *
 *   bun run packages/typescript/broker/test/claude/test-claude-hooks-surface.ts   (exit 0 = all pass)
 */
export {};
import { mkdtempSync, writeFileSync, mkdirSync, rmSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { claudeSessionId, claudeActivityDir, ClaudeObserveConnection } from '../../../adapters/claude/src/index.ts';
