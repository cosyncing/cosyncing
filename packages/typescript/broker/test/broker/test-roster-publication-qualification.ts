...
const boundaryModule = await import('../../src/roster/roster-publication.ts')
  .catch(() => undefined);
if (boundaryModule) {
  ...
  // packages/typescript/broker/src/runtime/runtime.ts at 1bc738a, verbatim. Only reachable when the
...
if (boundaryModule) {
  const runtimeSource = readFileSync(
    join(REPO, 'packages', 'typescript', 'broker', 'src', 'runtime', 'runtime.ts'),
    'utf8',
  );
