 ...
 *   phase `adapters`    — cross-adapter opt-in manifest (packages/typescript/broker/test/session-truth-conformance.json),
...
} from '../../src/roster/roster-publication.ts';
...
import type { CodexTuiScan } from '../../../adapters/codex/src/tui-presence.ts';
import { codexBaselineSourceIntact } from '../../../adapters/codex/src/run-state-repair.ts';
...
try {
  ...
  const adaptersPhaseStart = failures;
  {
    ...
    const graph = JSON.parse(
      readFileSync(resolve(import.meta.dir, '../../../../../scripts/verification/verification-graph.json'), 'utf8'),
