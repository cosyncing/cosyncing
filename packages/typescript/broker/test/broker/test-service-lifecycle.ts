...
import { collectDoctorReport } from '../../src/installation/doctor.ts';
import { inspectRepair, runRepair } from '../../src/installation/broker-lifecycle.ts';
import { inspectInstallState } from '../../src/installation/install-state.ts';
...
try {
  ...
  // only thing that shows it: the wall time of a child that performs one fast run() and nothing else.
  {
    ...
    const runnerModule = join(import.meta.dir, '../../src/installation/service-manager.ts');
    writeFileSync(probe, [
  ...
  // test hermetic while preserving the real command names and startup paths.
  {
    ...
    const codexModule = join(import.meta.dir, '../../../adapters/codex/src/index.ts');
    const opencodeModule = join(import.meta.dir, '../../../adapters/opencode/src/index.ts');
    const piModule = join(import.meta.dir, '../../../adapters/pi/src/index.ts');
    const claudeModule = join(import.meta.dir, '../../../adapters/claude/src/index.ts');
    const serveModule = join(import.meta.dir, '../../../adapters/opencode/src/managed-server.ts');
    const managedStateModule = join(import.meta.dir, '../../src/runtime/managed-runtime-state.ts');
    writeFileSync(probe, [
      ...
      `import ${JSON.stringify(managedStateModule)};`,
