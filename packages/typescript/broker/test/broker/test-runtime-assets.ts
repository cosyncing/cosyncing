...
} from '../../../adapters/claude/src/index.ts';
import {
...
import { browserClientUrl } from '../../src/transport/http-contracts.ts';
import {
...
// Resolver staging and visible doctor failures are deterministic and directly testable.
{
  ...
  const brokerCliSource = readFileSync(join(ROOT, 'packages/typescript/broker/src/cli/cli.ts'), 'utf8');
  const brokerRuntimeSource = readFileSync(join(ROOT, 'packages/typescript/broker/src/runtime/runtime.ts'), 'utf8');
