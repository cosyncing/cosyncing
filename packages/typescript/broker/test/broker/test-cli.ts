...
import { runCli, type BrokerRuntimeHandle, type CliDependencies } from '../../src/cli/cli.ts';
import {
...
import { committedInstallState } from '../../src/installation/install-state.ts';
...
import type { SetupCheck } from '../../../adapter-api/src/index.ts';
...
async function exerciseForeground(
  ...
  const child = Bun.spawn(command, {
    ...
    env: {
      ...
      COSYNCING_OMP_INTEGRATION_FILE: join(home, 'secrets', 'omp-integration.json'),
      HOST: '127.0.0.1',
      PORT: String(port),
    },
    stdin: 'ignore',
...
try {
  ...
  const imported = await runProcess([
    'bun',
    '-e',
    "await import('./packages/typescript/broker/src/cli/cli.ts')",
  ], {
    HOME: importHome,
