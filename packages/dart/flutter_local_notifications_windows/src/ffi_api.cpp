...
// cosyncing: every toast carries this group. An unpackaged app can only remove
// one toast through History.Remove(tag, group, aumid), which rejects an empty
// group, and a same tag and group replaces the earlier toast.
static const winrt::hstring kToastGroup = L"cosy";
...
bool showNotification(NativePlugin* plugin, int id, char* xml, NativeStringMap bindings) {
  ...
  notification.Group(kToastGroup);
...
void cancelNotification(NativePlugin* plugin, int id) {
  if (!plugin->isReady) return;
  const auto tag = winrt::to_hstring(id);
  // cosyncing: upstream removed toasts only with package identity, so an
  // unpackaged app's cancel was a no-op. Removing a toast that is no longer
  // in Action Center throws "Element not found"; that is not a failure.
  try {
    if (plugin->hasIdentity) {
      plugin->history.value().Remove(tag, kToastGroup);
    } else {
      plugin->history.value().Remove(tag, kToastGroup, plugin->aumid);
    }
...
int getNotificationSetting(NativePlugin* plugin) {
  if (!plugin->isReady) return -1;
  try {
    return static_cast<int>(plugin->notifier.value().Setting());
  } catch (winrt::hresult_error const&) {
    return -1;
  }
}
