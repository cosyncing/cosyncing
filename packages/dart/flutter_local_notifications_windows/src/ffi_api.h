...
/// cosyncing: ToastNotifier.Setting — 0 enabled, 1 disabled for the app,
/// 2 disabled for the user, 3 disabled by group policy, 4 disabled by
/// manifest; -1 when the plugin is not ready or the read fails.
FFI_PLUGIN_EXPORT int getNotificationSetting(NativePlugin* plugin);
