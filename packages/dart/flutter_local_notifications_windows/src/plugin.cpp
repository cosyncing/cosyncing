...
/// cosyncing: lets COM start this executable when a toast is clicked while the
/// app is not running. COM launches it with `-Embedding` and delivers the
/// activation once RegisterCallback has registered the class object. Written
/// on every start, so the entry follows the app wherever an installer put it.
void RegisterLocalServer(const std::string& clsid) {
  std::wstring exePath(MAX_PATH, L'\0');
  DWORD length = 0;
  for (;;) {
    length = GetModuleFileNameW(nullptr, exePath.data(), static_cast<DWORD>(exePath.size()));
    if (length == 0 || length < exePath.size()) break;
  ...
  // Convert the whole path at once: utf8_to_wstring keeps the terminating
  // NUL inside the string, so anything appended after it is cut off by c_str().
  const std::wstring keyPath =
    utf8_to_wstring("Software\\Classes\\CLSID\\" + clsid + "\\LocalServer32");
  RegistryKey key;
  winrt::check_win32(RegCreateKeyExW(
    HKEY_CURRENT_USER, keyPath.c_str(), 0, nullptr, 0, KEY_WRITE, nullptr, key.put(), nullptr
  ));
  winrt::check_win32(RegSetValueExW(
    key.get(), nullptr, 0, REG_SZ, reinterpret_cast<const BYTE*>(command.c_str()),
