...
class DesktopDrop {
  ...
  /// Fork addition: bounds how many paths one drop may deliver.
  ///
  /// Native code stops enumerating at [limit], so a folder-sized drop is never
  /// converted, marshalled, or allocated in this isolate. Pass one more than
  /// the caller's own selection limit — enough to prove overflow, never more.
  /// A negative [limit] restores upstream's unbounded delivery.
  ///
  /// Not supported on web, where the browser hands the whole `FileList` to the
  /// registrar; bound that at the call site instead.
  Future<void> setFileLimit(int limit) async {
    if (UniversalPlatform.isWeb) return;
    try {
      await _channel.invokeMethod<void>('setFileLimit', <String, Object>{
        'limit': limit,
      });
    } on MissingPluginException {
      // A host without the fork's handler keeps upstream's unbounded
      // delivery rather than failing the drop surface outright.
    }
  }
