...
namespace {
    ...
    // keeps upstream's unbounded enumeration. Written by the `setFileLimit`
    // method call and read by Drop, both on the platform thread.
    int64_t file_limit = -1;

    int64_t IntArgument(const flutter::EncodableValue *arguments,
                        const char *name) {
        const auto *map = std::get_if<flutter::EncodableMap>(arguments);
        if (!map) {
            return -1;
        }
        auto entry = map->find(flutter::EncodableValue(std::string(name)));
        if (entry == map->end()) {
            return -1;
        }
    ...
    void DesktopDropPlugin::RegisterWithRegistrar(
        ...
        channel->SetMethodCallHandler([](const auto &call, auto result) {
            ...
            if (call.method_name() == "setFileLimit") {
                file_limit = IntArgument(call.arguments(), "limit");
                result->Success();
                return;
            }
        ...
        if (pDataObj->QueryGetData(&fmtetc) == S_OK) {
            ...
            if (pDataObj->GetData(&fmtetc, &stgmed) == S_OK) {
                ...
                if (data != nullptr) {
                    ...
                    // Fork addition. DragQueryFile reports the count without
                    // reading a single name, so the bound applies before any
                    // path is converted or marshalled.
                    if (file_limit >= 0 && static_cast<int64_t>(files) > file_limit) {
                        files = static_cast<UINT>(file_limit);
                    }
                    for (unsigned int i = 0; i < files; ++i) {
                        TCHAR filename[MAX_PATH];
                        DragQueryFile(reinterpret_cast<HDROP>(data), i, filename, sizeof(TCHAR) * MAX_PATH);
                        std::wstring wide(filename);
                        // Fork divergence: upstream printed every dropped path
                        // to stdout.
                        list.push_back(flutter::EncodableValue(ws2s(wide)));
                    }
