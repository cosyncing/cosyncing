...
export 'src/pasteboard_platform.dart' show PasteboardLimitExceeded;
...
class Pasteboard {
  ...
  static Future<List<String>> files({int? limit}) =>
      pasteboard.files(limit: limit);

  /// Fork addition. Reads a clipboard image only if it fits in [maxBytes],
  /// refusing in native code before the encode and transport.
  static Future<Uint8List?> boundedImage({
    required int maxBytes,
    required int maxDecodedBytes,
  }) =>
      pasteboard.boundedImage(
        maxBytes: maxBytes,
