...
void PasteboardPlugin::HandleMethodCall(
  ...
  auto int_argument = [&method_call](const char *name) -> int64_t {
    const auto *arguments =
        std::get_if<flutter::EncodableMap>(method_call.arguments());
    if (!arguments) {
      return -1;
    }
    auto entry = arguments->find(flutter::EncodableValue(std::string(name)));
    if (entry == arguments->end()) {
      return -1;
    }
    if (const auto *wide = std::get_if<int64_t>(&entry->second)) {
      return *wide;
    }
    if (const auto *narrow = std::get_if<int32_t>(&entry->second)) {
      return *narrow;
    }
    return -1;
  };
  ...
  if (method_call.method_name() == "image") {
    ...
    // Fork addition. The DIB header carries the dimensions, so the decoded
    // size is known before any pixels are copied. Refuse an over-bound image
    // here: no HBITMAP allocation, no blit, no temporary BMP written to disk,
    // and nothing for Dart to read back.
    //
    // Widen before negating: biHeight is a LONG, and negating LONG_MIN is
    // signed overflow. The header is written by whichever app owns the
    // clipboard, so it is not ours to assume well formed.
    const int64_t width = bitmap->bmiHeader.biWidth;
    const int64_t signed_height = bitmap->bmiHeader.biHeight;
    const int64_t height = signed_height < 0 ? -signed_height : signed_height;
    // Fail closed on a malformed header rather than handing nonsense to GDI:
    // a non-positive dimension is not an image, and passing it on would skip
    // every bound below on the way to CreateHBitmapXRGB8888. Answering "no
    // image" matches how an unsupported bit depth is handled above, and lets
    // the paste fall through to whatever else the clipboard holds.
    if (width <= 0 || height <= 0) {
      result->Success();
      return;
    }

    // CreateHBitmapXRGB8888 below materializes 4 bytes per pixel. Compared by
    // division rather than multiplication: two dimensions near LONG_MAX
    // overflow width * height * 4 back into a value that would pass.
    //
    // `exceeds` answers "would width x height x 4, plus `overhead`, be more
    // than `limit`" without ever forming the product.
    auto exceeds = [&](int64_t limit, int64_t overhead) {
      if (limit < overhead) return true;
      return width > ((limit - overhead) / 4) / height;
    };

    // The two bounds are checked separately because they price different
    // things. The allocation ceiling covers the HBITMAP this is about to
    // create: pixels only.
    const int64_t decoded_bytes = int_argument("maxDecodedBytes");
    if (decoded_bytes >= 0 && exceeds(decoded_bytes, 0)) {
      result->Error("image-too-large",
                    "decoded clipboard image exceeds the caller bound");
      return;
    }
    // The result bound covers what Dart actually receives, which on this
    // platform is a BMP file: the pixels plus a BITMAPFILEHEADER and a
    // BITMAPINFOHEADER. CreateHBitmapXRGB8888 always produces 32bpp, so
    // CreateBitmapInfoStruct emits no colour table. Those 54 bytes are the
    // difference between passing a 64 MiB bound and delivering a file over it:
    // 4096x4096 is exactly 64 MiB of pixels, and Dart would read the whole
    // thing back before rejecting it.
    const int64_t result_bytes = int_argument("maxBytes");
    const int64_t bmp_overhead =
        static_cast<int64_t>(sizeof(BITMAPFILEHEADER)) +
        static_cast<int64_t>(sizeof(BITMAPINFOHEADER));
    if (result_bytes >= 0 && exceeds(result_bytes, bmp_overhead)) {
      result->Error("image-too-large",
                    "clipboard image exceeds the caller bound");
      return;
    }
    ...
    const int64_t limit = int_argument("limit");
    if (handle) {
      auto data = reinterpret_cast<HDROP>(GlobalLock(handle));
      if (data) {
        auto files = DragQueryFile(data, 0xFFFFFFFF, nullptr, 0);
        // Fork addition. DragQueryFile reports the count without touching a
        // single path, so the bound applies before any name is read out.
        if (limit >= 0 && static_cast<int64_t>(files) > limit) {
          files = static_cast<UINT>(limit);
        }
