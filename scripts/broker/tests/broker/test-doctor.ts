...
import { diagnoseManagedRuntimeFailure } from '../../../../packages/typescript/adapter-api/src/index.ts';
import {
  CODEX_MINIMUM_VERSION,
  CODEX_STANDALONE_INSTALL_COMMAND,
  diagnoseCodexSetup,
} from '../../../../packages/typescript/adapters/codex/src/diagnostics.ts';
import {
  OPENCODE_MINIMUM_VERSION,
  diagnoseOpenCodeSetup,
} from '../../../../packages/typescript/adapters/opencode/src/diagnostics.ts';
import {
  PI_MINIMUM_VERSION,
  diagnosePiSetup,
} from '../../../../packages/typescript/adapters/pi/src/diagnostics.ts';
import {
  CLAUDE_MINIMUM_VERSION,
  diagnoseClaudeSetup,
} from '../../../../packages/typescript/adapters/claude/src/diagnostics.ts';
...
} from '../../../../packages/typescript/broker/src/runtime-failure-store.ts';
import {
...
function literalDoctorCopy(): Array<{ file: string; text: string }> {
  ...
  const files = [
    'packages/typescript/broker/src/doctor.ts',
...
try {
  ...
  const codexFailure = await diagnoseCodexSetup(fakeContext({
  ...
  const codexFailureCheck = checkById(codexFailure, 'codex.managed-start-failure');
  check('Codex swallowed daemon-start failure is blocking, actionable, and captured output stays private',
