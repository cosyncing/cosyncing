 ...
 * adapter constructor and the broker-managed serve (broker/src/opencode-serve.ts) use to resolve OPENCODE_URL.
 ...
 */
import { resolveLocalOpencodeBaseUrl } from '../../../../packages/typescript/adapters/opencode/src/index.ts';
...
for (const url of ['http://127.0.0.1', 'http://localhost', 'http://127.0.0.1:4096', 'http://127.0.0.1:7000']) {
  ...
  const serveBase = resolveLocalOpencodeBaseUrl(url.replace(/\/$/, '')); // opencode-serve.ts trims then calls the same helper
