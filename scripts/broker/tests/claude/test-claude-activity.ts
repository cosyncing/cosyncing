 ...
 *
 *   bun run scripts/broker/tests/claude/test-claude-activity.ts      (exit 0 = all pass)
 */
export {};
import { mkdirSync, writeFileSync, rmSync, existsSync, utimesSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { buildActivitySnapshot, collectParentActivity, claudeActivityDir, ClaudeActivityWatcher, mapTranscript } from '../../../../packages/typescript/adapters/claude/src/index.ts';
