 ...
 *
 *   bun run scripts/broker/tests/claude/test-claude-control.ts      (exit 0 = all pass)
 */
export {};
import { mkdirSync, writeFileSync, rmSync, existsSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { bridgedUuids, claudeControl, eligibleForChannels, resumeArgs, resumeEnv, readLatestModel, claudeModelOptions, CLAUDE_MAX_MODEL_OPTIONS, CLAUDE_PERMISSION_MODES, ClaudeAdapter, modelAlias, resolveWrapperModels, liveTerminalOwner } from '../../../../packages/typescript/adapters/claude/src/index.ts';
